...
schools = json.load(infile) #making the json file into a dictionary
conferance_schools = [372,108,107,130] # creating a list from the school data above
...
for school in schools:
    if school['NCAA']["NAIA conference number football (IC2020)"] in conferance_schools:
...
for school in schools:
    if school['NCAA']["NAIA conference number football (IC2020)"] in conferance_schools:
        if school["Total price for in-state students living on campus 2020-21 (DRVIC2020)"] is not None: # had to deal with null values
            if school["Total price for in-state students living on campus 2020-21 (DRVIC2020)"] > 50000:
                ...
                print(f"Total price for in-state students living on campus 2020-21: ${school['Total price for in-state students living on campus 2020-21 (DRVIC2020)']}")
